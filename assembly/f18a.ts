    ...
    switch (screenMode) {
        ...
        case MODE_TEXT_80:
            if (x >= borderWidth && x < drawWidth - borderWidth) {
                // In text modes the name and attribute addresses are separate counters:
                // name = NTBA + text position, attribute = CTBA + text position. The name
                // table base must not leak into the attribute address (F18A f18a_tiles.vhd).
